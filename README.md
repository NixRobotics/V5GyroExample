...
commands within user_control() or autonomous() from executing. This will introduce a small delay before the robot can move.
...
In general mounting tips are as follows:
 ...
 - Avoid jolts during autonomous routines: The sensors will "saturate" when bumped meaning that the motion is to large for the
   output reading. If "wall bumps" are part of the autonomous routine to straighten out the robot, then reset the heading in between
   these
